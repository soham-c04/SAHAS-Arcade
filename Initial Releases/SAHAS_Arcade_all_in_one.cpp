...
//Air Hockey
	...
	int X=10,Y=100*Yratio;
	int goal_width=90*Xratio,goal_height=125*Yratio;
	...
	int x1=0,yy1=0;//coordinates of first striker
	...
	void goaling_AH(int who)//checks goal condition and updates value if goal is scored
	{
		...
		if(who==1)//gone into first players goal
		{
			...
			yy1=(Y2+Y1)/2;//reset striker 1 Y coordinate
	...
	{
		int ans=0;
		if(pow(x1-xb,2)+pow(yy1-yb,2)<pow(radius_striker+radius_ball,2))//collision of ball with first striker
			ans++;
		if(pow(x2-xb,2)+pow(y2-yb,2)<pow(radius_striker+radius_ball,2))//collision of ball with second striker
			ans+=2;
		if(pow(x1-x2,2)+pow(yy1-y2,2)<=pow(2*radius_striker,2))
			ans+=3;
		return ans;
	}
	void collision_AH()
	{
		if(pow(x1-xb,2)+pow(yy1-yb,2)<=pow(radius_striker+radius_ball,2))
		{
			double theta=deg(atan((yy1-yb)*1.0/(xb-x1)));
			if(yb==yy1)
			{ 
				theta=(x1>xb)*(180);
			}
	...
	void control_striker_AH()
	{
		...
		x1+=incx;
		yy1-=incy;
		yy1=max(yy1,Y1+radius_striker);
		yy1=min(yy1,Y2-radius_striker);
			...
			else if(GetAsyncKeyState(VK_RETURN))
			{
				switch(option)
				{
					...
					case 2:
						...
						x1=X1+radius_striker+10,yy1=(Y1+Y2)/2;
	...
	while(1)
	{
		...
		else if(GetAsyncKeyState(VK_RETURN))
		{
			switch(choice)
			{
				...
				case 2:
					X=10,Y=100*Yratio;
					goal_width=90*Xratio,goal_height=125*Yratio;
					cleardevice();
					page=1-page;
					setvisualpage(page);
					setactivepage(1-page);
